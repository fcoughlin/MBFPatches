...
namespace MBFPatches.Patches
{
    ...
    class IntroCutsceneOnDestroyPatch
    {
        public static void Prefix(IntroCutscene __instance)
        {
            ...
            if (MBFPatchesPlugin.RandomSpawns.Value && PlayerControl.LocalPlayer != null)
            {
                ...
                if (mapID == 0)
                    coords = skeldCoords[(MBFPatches.rnd.Next(skeldCoords.Count) + PlayerControl.LocalPlayer.PlayerId) % skeldCoords.Count()];
                else if (mapID == 1)
                    coords = miraCoords[(MBFPatches.rnd.Next(miraCoords.Count) + PlayerControl.LocalPlayer.PlayerId) % miraCoords.Count()];
                else if (mapID == 2)
                    coords = polusCoords[MBFPatches.rnd.Next((polusCoords.Count) + PlayerControl.LocalPlayer.PlayerId) % polusCoords.Count()];
